...
class PacketContractValidationTests(unittest.TestCase):
    ...
    def test_p6_17_contract_validation_covers_all_packet_actors(self) -> None:
        for actor in ("codex", "reviewer", "judge"):
            with self.subTest(actor=actor):
                payload = packet_contract_validation_payload("P6-17", "lowest-cost", actor)

                self.assertTrue(payload["valid"])
                self.assertEqual(payload["schema_version"], CONTRACT_SCHEMA_VERSION)
                self.assertEqual(payload["objective"], "P6-17")
                self.assertEqual(payload["profile"], "lowest-cost")
                self.assertEqual(payload["actor"], actor)
                self.assertEqual(
                    payload["packet_identity"],
    ...
    def test_p6_18_contract_validation_covers_all_packet_actors(self) -> None:
        for actor in ("codex", "reviewer", "judge"):
            with self.subTest(actor=actor):
                payload = packet_contract_validation_payload("P6-18", "lowest-cost", actor)

                self.assertTrue(payload["valid"])
                self.assertEqual(payload["schema_version"], CONTRACT_SCHEMA_VERSION)
                self.assertEqual(payload["objective"], "P6-18")
                self.assertEqual(payload["profile"], "lowest-cost")
                self.assertEqual(payload["actor"], actor)
                self.assertEqual(
                    payload["packet_identity"],
